...
import {
  Message,
  MessageDocument,
} from '../src/messages/schemas/message.schema';
...
describe('Rag Ask (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.MONGODB_URI =
      process.env.MONGODB_URI ??
      'mongodb://127.0.0.1:27017/rag-platform-rag-ask-e2e';
    process.env.DOCUMENTS_UPLOAD_DIR =
      process.env.DOCUMENTS_UPLOAD_DIR ??
      join(process.cwd(), 'uploads', 'rag-ask-e2e');
  ...
  it('completes full rag ask flow and persists messages with citations and trace', async () => {
    ...
    const conversationLastMessageAtBefore = new Date(
    ...
    const persistedConversation = await conversationModel
      .findById(conversation.id)
      .exec();
    expect(persistedConversation).not.toBeNull();
    expect(
      persistedConversation!.lastMessageAt.getTime(),
    ).toBeGreaterThanOrEqual(conversationLastMessageAtBefore);
  ...
  it('stores actual executed retrieval provider in response and assistant trace', async () => {
    ...
    const assistantMessage = await messageModel
      .findById(body.assistantMessageId)
      .exec();
