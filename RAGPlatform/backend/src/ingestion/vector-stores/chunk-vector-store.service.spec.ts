...
describe('ChunkVectorStoreService', () => {
  ...
  it('maps vector search rows and filters null mapping results', async () => {
    ...
    expect(vectorStoreMock.similaritySearchVectorWithScore).toHaveBeenCalledWith(
      [0.2, 0.4],
      2,
      {
        preFilter: {
  ...
  it('rejects empty query embedding before vector search', async () => {
    ...
    expect(vectorStoreMock.similaritySearchVectorWithScore).not.toHaveBeenCalled();
  });
});
