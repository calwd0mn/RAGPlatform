...
export class RagService {
  ...
  async invokeLlmForWorkflow(input: {
    ...
    try {
      ...
      return {
        text,
        json:
          input.outputMode === 'json' ? this.tryParseWorkflowJson(text) : null,
      ...
      throw new InternalServerErrorException(
        'Failed to invoke workflow LLM node',
      );
    }
  }

  async rewriteQueryForWorkflow(input: { query: string }): Promise<{
  ...
  private async executeAsk(
    ...
    try {
      ...
      const createdAssistantMessage =
        await this.createAssistantMessageForRequest({
          userId,
          conversationId: dto.conversationId,
  ...
  private tryParseWorkflowJson(
    ...
    try {
      ...
      const normalizedEntries = Object.entries(record).flatMap(
        ([key, value]) => {
          if (
            value === null ||
            typeof value === 'string' ||
  ...
  private scoreBm25LikeChunk(
    ...
    queryTokens.forEach((token) => {
      ...
      const inverseDocumentFrequency = Math.log(
        1 +
          (totalDocumentCount - documentFrequency + 0.5) /
  ...
  private async createAssistantMessageForRequest(input: {
    ...
    if (input.requestId) {
      ...
      if (existingMessage) {
        if (
          existingMessage.conversationId.toString() !== input.conversationId
        ) {
          throw new BadRequestException(
            'requestId has been used by another conversation',
          );
