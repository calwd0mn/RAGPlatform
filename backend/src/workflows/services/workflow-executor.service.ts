...
export class WorkflowExecutorService {
  ...
  async execute(input: {
    ...
    const nodeMap = new Map(
      nodes.map((node): [string, WorkflowNode] => [node.id, node]),
    );
  ...
  private async executeNode(input: {
    ...
    switch (input.node.type) {
      ...
      case 'mergeResults':
        return this.executeMergeResultsNode(
          input.node,
          input.context,
          input.incomingNodeIds,
        );
      case 'rerank':
        return this.executeRerankNode(
          input.node,
          input.context,
          input.incomingNodeIds,
        );
  ...
  private async executeAnswerNode(input: {
    ...
    const latestRetrieval = retrievalOutputs.at(-1);
    const question = this.resolveTemplate(
      input.node.data.question,
      input.context,
    );
    ...
    const answer: WorkflowRagAnswerResult =
      await this.ragService.answerWorkflow({
        knowledgeBaseId: input.knowledgeBaseId,
  ...
  private executeConditionNode(
    ...
    const result = node.data.conditions.every((condition): boolean =>
    ...
    if (input.node.type === 'condition') {
      ...
      downstream.forEach((edge): void => {
        if (edge.sourceHandle === activeHandle) {
          this.enqueueWhenReady(
            edge.target,
            input.runtimeInDegree,
            input.queue,
          );
          return;
        }
  ...
  private findRetrievalOutputsFromInputs(
    ...
    return incomingNodeIds
      ...
      .filter((output): output is WorkflowRagOutput =>
        this.isRagOutput(output),
      );
  }
  ...
  private resolveFinalOutput(context: WorkflowExecutionContext): string {
    const outputs = Object.values(context).filter(
      (output): output is WorkflowOutputNodeOutput =>
  ...
  private toRagOutput(
    retrieval: WorkflowRagRetrievalResult,
  ): WorkflowRagOutput {
    return {
